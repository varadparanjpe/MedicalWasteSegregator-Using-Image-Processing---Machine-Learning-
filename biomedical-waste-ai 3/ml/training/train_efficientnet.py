"""
Train EfficientNet-B0 on the preprocessed dataset from MLA_PreProcessing.py.

Features:
  - Optional inverse-frequency class-weighted cross-entropy (--class-weights)
  - Cosine LR schedule, AdamW, best-by-F1 checkpointing
  - On finish: confusion matrix + per-class + latency → DB (dashboard-ready)
...
def main():
    ...
    model = build_efficientnet(num_classes=len(CATEGORIES), pretrained=True).to(dev)
    ...
    # ─── Test-set evaluation + persist everything for the dashboard ────────
    ...
    db.init_db()
    db.save_model_metrics({"accuracy": acc, "precision": pr, "recall": rc, "f1": f1, "per_class": per_class})
    cm = confusion_matrix(y_true, y_pred, labels=list(range(len(CATEGORIES))))
    db.save_confusion_matrix(CATEGORIES, cm.tolist(), model="efficientnet_b0")
